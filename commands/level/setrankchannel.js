...
module.exports = {
    config: {
        name: 'setrankchannel',
        group: "level",
        description: 'Sets a Rank Channel',
        aliases: ["rankchannel"], 
        usage: '<channel>',
...
if (!args[0]) {
  let b = await db.fetch(`rank_${message.guild.id}`);
  let channelName = message.guild.channels.cache.get(b);
  if (message.guild.channels.cache.has(b)) {
    return message.channel.send(
      `**${emote.verified} Rank Channel Set In This Server Is \`${channelName.name}\`!**`
    );
  } else
    ...
    try {
        let a = await db.fetch(`rank_${message.guild.id}`)

        if (channel.id === a) {
            return message.channel.send({embed: {
            color: config.embedcolor,
            title: `${emote.info} This Channel is already set as rank Channel!`
        }})
        } else {
            client.guilds.cache.get(message.guild.id).channels.cache.get(channel.id).send(`**${emote.verified} Rank Channel Set!**`)
            db.set(`rank_${message.guild.id}`, channel.id)

           message.channel.send({embed: {
            color: config.embedcolor,
            title: `${emote.verified} Rank Channel has been Set Successfully \`${channel.id}\``
        }})
        }
    } catch {
