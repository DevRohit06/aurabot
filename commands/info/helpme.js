...
${pu.config.name ? `**Name:** ${pu.config.name}` : ""}${pu.config.description ? '\n' : ""}${pu.config.description ? `**Description:** ${pu.config.description}` : ""}${pu.config.aliases ? '\n' : ""}${pu.config.aliases ? `**Aliases:** ${pu.config.aliases.join(', ')}` : ""}${pu.config.group ? '\n' : ""}${pu.config.group ? `**Group:** ${pu.config.group}` : ""}${pu.config.permissions ? '\n' : ''}${pu.config.permissions ? `**Permissions:** ${pu.config.permissions.join(', ').toLocaleLowerCase()}` : ""}${pu.config.usage ? '\n' : ""}${pu.config.usage ? `**Usage:** ${pu.config.usage}` : ""}${pu.config.example ? "\n" : ""}${pu.config.example ? `**Example:** ${pu.config.example}` : ""}                             
    ...
    message.channel.send(embed)
     ...
     if (args[0] === 'fun') {
        ...
        client.commands.forEach((command) => {
            if (command.config.group === 'fun') fun.push(`\`${command.config.name}\` `);
        })
        ...
        let action = [];
        client.commands.forEach((command) => {
            if (command.config.group === 'action') action.push(`\`${command.config.name}\` `);
        })
     ...
     if (args[0] === 'giveaway') {
        let give = [];
        client.commands.forEach((command) => {
            if (command.config.group === 'giveaway') give.push(`\`${command.config.name}\``);
        })
      ...
      if (args[0] === 'ticket') {
        let ticket = [];
        client.commands.forEach((command) => {
            if (command.config.group === 'ticket') ticket.push(`\`${command.config.name}\` `);
        })
     ...
     if (args[0] === 'anime') {
         ...
         client.commands.forEach((command) => {
            if (command.config.group === 'anime') misc.push(`\`${command.config.name}\` `);
        })
     ...
     if (args[0] === 'level') {
        ...
        client.commands.forEach((command) => {
           if (command.config.group === 'level') level.push(`\`${command.config.name}\` `);
       })
     ...
     if (args[0] === 'chatbot') {
        ...
        client.commands.forEach((command) => {
           if (command.config.group === 'chatbot') mnge.push(`\`${command.config.name}\` `);
       })
     ...
     if (args[0] === 'backup') {
        ...
        client.commands.forEach((command) => {
           if (command.config.group === 'backup') backup.push(`\`${command.config.name}\` `);
       })
     ...
     if (args[0] === 'image') {
         ...
         client.commands.forEach((command) => {
            if (command.config.group === 'images') configuration.push(`\`${command.config.name}\` `);
        })
        ...
        message.channel.send(configEmbed);
         ...
         client.commands.forEach((command) => {
            if (command.config.group === 'games') games.push(`\`${command.config.name}\` `);
        })
        ...
        if (args[0] === 'economy') {
         ...
         client.commands.forEach((command) => {
            if (command.config.group === 'economy') economy.push(`\`${command.config.name}\` `);
        })
    ...
    if (args[0] === 'info' || args[0] === 'information') {
        let infoo = [];
        client.commands.forEach((command) => {
            if (command.config.group === 'info') infoo.push(`\`${command.config.name}\` `);
        })
    ...
    if (args[0] === 'other') {
        ...
        client.commands.forEach((command) => {
            if (command.config.group === 'other') {
                other.push(`\`${command.config.name}\` `);
            }
            });
    ...
    if (args[0] === 'setup') {
        ...
        client.commands.forEach((command) => {
            if (command.config.group === 'setup') {
                setup.push(`\`${command.config.name}\` `);
            }
            });
